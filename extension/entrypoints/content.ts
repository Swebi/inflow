export default defineContentScript({
  ...
  main() {
    ...
    console.log(
      "[Content Script] ✅ Gmail content script LOADED on:",
      ...
      (message: any, sender: any, sendResponse: (response: any) => void) => {
        ...
        if (message.action === "extractEmail") {
          ...
          try {
            ...
            console.log(
              "[Content Script] Email extracted successfully, length:",
              emailContent.length
            );
            console.log(
              "[Content Script] Email content preview:",
...
function setupGmailNavigationDetection(): void {
  try {
    ...
    const handleNavigation = (): void => {
      try {
        ...
        if (currentUrl !== lastUrl) {
          lastUrl = currentUrl;
          console.log(
            "[Content Script] 🔄 Gmail navigation detected:",
            currentUrl
          );
          ...
          debounceTimer = window.setTimeout(() => {
            console.log(
              "[Content Script] Navigation complete, ready for extraction"
            );
            ...
            setTimeout(() => {
              if (isViewingEmail()) {
                console.log(
                  "[Content Script] 📧 Email view detected, notifying sidepanel..."
                );
                ...
                browser.runtime
                  ...
                  .catch((error) => {
                    ...
                    console.log(
                      "[Content Script] No listener for emailChanged message (sidepanel may be closed)"
                    );
                  });
              } else {
                console.log(
                  "[Content Script] Not viewing an email, skipping notification"
                );
              }
            }, 300);
          }, 500);
    ...
    console.error(
      "[Content Script] Error setting up navigation detection:",
      error
    );
  }
}
...
function extractGmailEmail(): string {
  ...
  if (!isViewingEmail()) {
    console.warn(
      "[Content Script] Not viewing an email, user is on inbox/list view"
    );
    throw new Error("Open an email to get started");
  }
  ...
  console.log(
    "[Content Script] Method 2: Looking for .a3s email body (without .aiL)..."
  );
  const emailBodiesAlt = document.querySelectorAll(".a3s");
  console.log(
    "[Content Script] Found",
    emailBodiesAlt.length,
    "email bodies with .a3s"
  );
  ...
  if (emailArticles.length > 0) {
    ...
    if (messageBody) {
      ...
      if (text.length > 10) {
        console.log(
          "[Content Script] Found message body in article, length:",
          text.length
        );
        return text;
      }
  ...
  if (threadContainer) {
    ...
    if (messageBody) {
      ...
      if (text.length > 10) {
        console.log(
          "[Content Script] Found message body in thread container, length:",
          text.length
        );
        return text;
      }
  ...
  throw new Error(
    "Could not extract email content. The email might still be loading, or Gmail's layout has changed. Please try refreshing the page."
  );
}
